...
def lnc_console():   
    ...
    if lnc_command == "story-mode":
        ...
        time.sleep(1)
        subprocess.run("start files/lnc.exe -iwad gamefiles.ipk3 -LNC_LAUNCHER" if os.name == "nt" else "", shell=True) # TODO: WRITE THE LINUX ONE AND MAC ONE
        # if ur looking into this source for making ur custom lnc launcher (weirdo) then u need to pass -LNC_LAUNCHER when launching the executable file, otherwise the game wont launch
        exit()
